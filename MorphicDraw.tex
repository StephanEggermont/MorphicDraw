...
\reversemarginpar
\marginpar{
{\color{blue}{\hrule}}{\  }\\[1ex]
\small
StandardWindow is the default entry point for Polymorph. It is a
subclass of SystemWindow. It adds support for the current UITheme,
running full-screen and adds a lot of factory methods to add 
(Poly)Morphs to the window.\\
\color{blue}{\hrule}}
...
and then
\begin{verbatim}
MDShape>>doDrag: aMouseMoveEvent with: aMDShape     ActiveHand addEventListener: self.    self setConstrainedPosition: 
            aMouseMoveEvent position - (self extent /2)
        hangOut: false.	
\end{verbatim}
...
\begin{verbatim}
MDShape>>click: evt
    "Default behavior: Show a context menu if right mouse button 
    was clicked."
